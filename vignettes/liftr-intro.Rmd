...
Now we can use `render_docker()` on `docker.Rmd` to render the document to a html file, under a Docker container:

```{r, eval = FALSE}
render_docker(docker_input)
```

The function `render_docker()` will parse the Dockerfile, build a new Docker image, and run a container to render the input document. If successfully rendered, the output `docker.html` will be in the `~/liftr_docker/` directory. You can also passed additional arguments in `rmarkdown::render` to this function.
