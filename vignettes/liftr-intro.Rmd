...
metadata format, and uses [Docker](https://www.docker.com) to containerize and
...
To clean up the (unused) Docker image after sucessful rendering,
you can use `purge_image()`:
...
the R Markdown documents. Here is a
[detailed guide](https://docs.docker.com/engine/installation/)
for installing Docker on major operation systems.

For Linux, we should configure Docker to [run without sudo](https://docs.docker.com/engine/installation/linux/linux-postinstall/).
