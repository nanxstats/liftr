...
## Optional options
  ...
  * `cranpkg` - CRAN packages depended in the document.
...
The Rabix options are optional. Just make sure `rabix: true` when you need to enable Rabix support.
  ...
  * `rabix_json` - The URI (local file path or HTTP URL) to a JSON document that describes the Rabix app.
