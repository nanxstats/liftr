...
- Added the new argument `no_cache` to improve rendering speed and fix [#18](https://github.com/road2stat/liftr/issues/18).
- Separate the creation and removal of Docker containers and images: removed the old argument `reset` in `drender()`; added `purge_container()`, `purge_image()`, and ``purge_all()` for cleaning up.
